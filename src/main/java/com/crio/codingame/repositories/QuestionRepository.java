...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        ...
        for(String x:questionMap.keySet()){
            allQuestions.add(questionMap.get(x));
        }
        return allQuestions;
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> questionLevelWise = new ArrayList<>();
        for(Question x:questionMap.values()){
            if(x.getLevel().equals(level)){
                questionLevelWise.add(x);
            }
        }
     return questionLevelWise;
    }
